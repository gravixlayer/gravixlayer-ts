...
const created = await client.runtimes.create({ template: TEMPLATE, timeoutSeconds: 600 });
...
const runtime = await client.runtimes.connect(savedId);
...
// the runtime is up.
const same = await client.runtimes.get(savedId);
