...
await runtime.files.write('/workspace/hello.txt', 'Hello from the SDK.\nSecond line.');
const hello = await runtime.files.read('/workspace/hello.txt');
console.log(`Read       : ${hello.content.split('\n')[0]}`);

// 2. Create a directory. Parents are created too, unless you say otherwise.
await runtime.files.createDirectory('/workspace/project/src');

// 3. Write a file into it and list the directory.
await runtime.files.write('/workspace/project/src/main.py', 'print("version 1.0")\n');
const listing = await runtime.files.list('/workspace/project/src');
...
await runtime.files.upload('/workspace/project/config.json', '{"debug": true, "port": 8080}');
...
const before = await runtime.files.getInfo('/workspace/project/run.sh');
console.log(`Info       : exists=${before.exists} mode=${before.info?.mode}`);

await runtime.files.setPermissions('/workspace/project/run.sh', '0755');
const after = await runtime.files.getInfo('/workspace/project/run.sh');
console.log(`After chmod: mode=${after.info?.mode}`);

// 7. Download bytes, or text if that is what the file holds.
const bytes = await runtime.files.download('/workspace/hello.txt');
console.log(`\nDownloaded : ${bytes.length} bytes`);
console.log(
  `As text    : ${(await runtime.files.downloadText('/workspace/hello.txt')).split('\n')[0]}`,
);

// 8. Move and copy. A directory needs `recursive`.
await runtime.files.move('/workspace/hello.txt', '/workspace/project/notes.txt');
await runtime.files.copy('/workspace/project/notes.txt', '/workspace/project/notes.bak');
await runtime.files.copy('/workspace/project/src', '/workspace/project/src-copy', {
  recursive: true,
});

// 9. Change ownership. Accepts names or numeric ids.
const owner = (await runtime.runCmd('id -un')).stdout.trim();
await runtime.files.chown('/workspace/project/src-copy', { user: owner, recursive: true });
console.log(`Owner      : /workspace/project/src-copy -> ${owner}`);

// 10. Search by filename, then by content. A pattern is literal text unless
//     you pass `regex: true`.
const byName = await runtime.files.find('/workspace/project', { glob: '*.py' });
console.log(`\nBy name    : ${byName.matches.length} file(s), scanned ${byName.filesScanned}`);

const byContent = await runtime.files.find('/workspace/project', { pattern: 'version' });
...
const preview = await runtime.files.replace('/workspace/project', '1.0', '2.0', {
  glob: '*.py',
  dryRun: true,
});
console.log(`\nDry run    : would change ${preview.totalReplacements} occurrence(s)`);

const applied = await runtime.files.replace('/workspace/project', '1.0', '2.0', { glob: '*.py' });
...
for await (const event of runtime.files.watch('/workspace/project')) {
  if (event.type === 'start') {
