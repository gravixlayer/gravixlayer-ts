...
console.log('\n--- callbacks ---');
const result = await sandbox.runCmd("sh -lc 'for i in 1 2 3; do echo line-$i; sleep 1; done'", {
...
console.log('\n--- for await ---');
for await (const event of sandbox.streamCmd("sh -lc 'echo one; echo two; echo three'")) {
...
console.log('\n--- streaming code ---');
for await (const event of sandbox.streamCode(
  'import time\nfor i in range(3):\n    print(f"step {i}")\n    time.sleep(0.5)',
)) {
